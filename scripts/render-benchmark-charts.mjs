...
function renderPanel({
  ...
  parts.push(
    `<text x="${LEFT_MARGIN - 12}" y="${originY - PANEL_HEIGHT - 10}" font-size="14" font-weight="600" fill="${COLORS.ink}" font-family="system-ui, -apple-system, sans-serif">${escapeXml(label)} (${escapeXml(unit)})</text>`,
  );
  ...
  names.forEach((name, index) => {
    ...
    const { parts: segParts, top } = renderDiffSegment({
      x,
      width: barWidth,
      baseY: originY,
      row,
    ...
    if (row.isNew) {
      parts.push(
        `<text x="${x + barWidth / 2}" y="${originY - top - 6}" font-size="10" text-anchor="middle" fill="${COLORS.inkSecondary}" font-family="system-ui, -apple-system, sans-serif">new</text>`,
      );
      return;
    }
    ...
    if (pct !== undefined) {
      ...
      parts.push(
        `<text x="${x + barWidth / 2}" y="${originY - top - 6}" font-size="10" text-anchor="middle" fill="${color}" font-family="system-ui, -apple-system, sans-serif">${sign}${pct.toFixed(0)}%</text>`,
      );
    }
  });
...
export function renderSuiteChart({ suiteTitle, metrics, showLabels = true }) {
  const metricRows = metrics.map((metric) => ({
    ...metric,
    rowsByName: new Map(
      computeDeltaRows(metric.entries, metric.baselineByName).map((row) => [row.name, row]),
    ),
  }));
  const names = [
    ...new Set(metricRows.flatMap((metric) => metric.entries.map((entry) => entry.name))),
  ];

  const width = Math.max(
    legendWidth(24, DIFF_LEGEND_ITEMS),
  ...
  metricRows.forEach((metric, index) => {
    ...
    const maxValue = Math.max(
      1,
      ...metric.entries.map((e) => Math.max(e.value, metric.baselineByName.get(e.name) ?? 0)),
    );
    panels.push(
      renderPanel({
        label: metric.label,
        unit: metric.unit ?? 'ms',
        rows: metric.rowsByName,
        names,
        originY,
        maxValue,
        barWidth,
...
export function renderDeltaTableMarkdown(rows) {
  ...
  lines.push(
    `| Avg | across ${withBaseline.length} test${withBaseline.length === 1 ? '' : 's'} with a baseline | | | ${avgSignNominal}${avgNominal.toFixed(0)} ms | ${avgSignPct}${avgPct.toFixed(1)}% |`,
  );
