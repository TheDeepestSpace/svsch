...
function aggregateWidth(node: DiagramNode, ctx: NodeSizingContext): number {
  ...
  const topLabelWidth =
    ctx.topPorts.length > 0
      ? (ctx.topPorts.length * 2 - 1) * diagramSizing.gridSize +
        Math.max(...ctx.topPorts.map((p) => measureText(p.label ?? p.name)))
      : 0;
  const bottomLabelWidth =
    ctx.bottomPorts.length > 0
      ? (ctx.bottomPorts.length * 2 - 1) * diagramSizing.gridSize +
        Math.max(...ctx.bottomPorts.map((p) => measureText(p.label ?? p.name)))
      : 0;
