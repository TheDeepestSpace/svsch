...
export function visualHandleGeometry(node: DiagramNode, portId: string | undefined): VisualHandleGeometry | undefined {
...
function busSingleHandleGeometry(node: DiagramNode, port: DiagramPort): VisualHandleGeometry | undefined {
  const role = structRole(node);
  const inputs = node.ports.filter((candidate) => candidate.direction === 'input' || candidate.direction === 'inout' || candidate.direction === 'unknown');
...
function interfaceInstanceScalarHandleGeometry(node: DiagramNode, port: DiagramPort): VisualHandleGeometry | undefined {
...
export function interfaceInstanceTopHatY(node: DiagramNode, height: number): number {
  ...
  const topPorts = aggregatePorts.filter((candidate) => candidate.direction === 'input' && candidate.width !== 'interface');
  const bottomPorts = aggregatePorts.filter((candidate) => candidate.direction === 'output' && candidate.width !== 'interface');
  const sidePorts = aggregatePorts.filter((candidate) => candidate.width === 'interface' || (candidate.direction !== 'input' && candidate.direction !== 'output'));
