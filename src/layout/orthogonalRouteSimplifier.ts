...
function findSimplification(
  ...
  for (let index = 0; index + 3 < route.length; index += 1) {
    ...
    if (first.length <= maxExcursion + GEOMETRY_EPSILON) {
      ...
      alternateElbows.push(
        first.axis === 'vertical' ? { x: firstTurn.x, y: end.y } : { x: end.x, y: firstTurn.y },
      );
    }
    if (last.length <= maxExcursion + GEOMETRY_EPSILON) {
      elbows.push(
        last.axis === 'vertical' ? { x: firstTurn.x, y: end.y } : { x: end.x, y: firstTurn.y },
      );
      alternateElbows.push(
        last.axis === 'vertical'
          ? { x: secondTurn.x, y: start.y }
    ...
    const candidatesFor = (candidateElbows: OrthogonalRoutePoint[]) =>
      candidateElbows
        .filter(
          (elbow, elbowIndex) =>
        ...
        .map((elbow) =>
          removeRedundantPoints([...route.slice(0, index + 1), elbow, ...route.slice(index + 3)]),
        )
...
function segmentsCross(first: RouteSegment, second: RouteSegment): boolean {
  const horizontal = first.axis === 'horizontal' ? first : second;
  const vertical = first.axis === 'vertical' ? first : second;
  return (
    within(horizontal.start.x, horizontal.end.x, vertical.start.x) &&
...
function segmentVector(
  ...
  if (start.x === end.x && start.y !== end.y) {
    return {
      axis: 'vertical',
      direction: Math.sign(end.y - start.y),
  ...
  if (start.y === end.y && start.x !== end.x) {
    return {
      axis: 'horizontal',
      direction: Math.sign(end.x - start.x),
...
function endpointDirectionsMatch(
  ...
  const candidateEnd = segmentVector(
    candidate[candidate.length - 2],
    candidate[candidate.length - 1],
  );
  return (
    originalStart.axis === candidateStart.axis &&
...
function removeRedundantPoints(route: OrthogonalRoutePoint[]): OrthogonalRoutePoint[] {
  const deduplicated = route.filter(
    (point, index) => index === 0 || !pointsEqual(point, route[index - 1]),
  );
...
function routeLength(route: OrthogonalRoutePoint[]): number {
  return route
    .slice(1)
    .reduce((total, point, index) => total + segmentLength(route[index], point), 0);
}
