...
TEST(ExtractorTest, SyncResetDoesNotFalsePositiveWhenNoDefaultNameMatches) {
    namespace fs = std::filesystem;
    ...
    vpiHandle design = restoredDesigns[0];
    svsch::DesignExtractor extractor(design);
    // "clr_n" matches neither default reset name ("rst"/"reset"). Unlike
    // clock/reset disambiguation in an async sensitivity list (where every
    // identifier present is necessarily a clock or reset signal), an if/else
    // condition can be an arbitrary boolean, so falling back positionally here
    // would misclassify ordinary conditional register updates (e.g. a plain mux
    // select) as synchronous resets. The extractor must not detect a sync reset
    // in this case.
    extractor.reset_signal_names = {"rst", "reset"};
    nlohmann::json result = extractor.extract();
