    ...
    for (const rawMod of raw.modules) {
        ...
        const module: DesignModule = {
            ...
            edges: (rawMod.edges || []).map((e, i) => {
                ...
                const isStructSignalEdge = hasStructTypedPort(sourceNode) || hasStructTypedPort(targetNode);
                ...
                const edge: DiagramEdge = {
                    ...
                    metadata: {
                        ...e.metadata,
                        aggregate: (isInterfaceInstanceSource || isInterfaceInstanceTarget) && (e.source !== 'self' && e.target !== 'self') ? 'interface' : (isStructComposition || isStructSignalEdge ? 'struct' : e.metadata?.aggregate)
                    }
                };
                return edge;
            })
