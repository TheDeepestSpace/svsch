...
describe.each(['uhdm'] as const)('nested case statements: %s', (backend) => {
  it('extracts nested case statements as nested muxes', async () => {
    ...
    expect(muxB.ports.some(p => p.label === "2'b00" && p.connectedSignal === 'data_in1')).toBe(true);
    expect(muxB.ports.some(p => p.label === "2'b01" && p.connectedSignal === 'data_in2')).toBe(true);
    ...
    expect(muxA.ports.some(p => p.label === "2'b00" && p.connectedSignal === muxBOutput)).toBe(true);
    ...
    expect(mod.edges.some(e => e.source === muxA.id && e.target === 'port:nested_case:data_out')).toBe(true);
  });
  ...
  it('handles missing branches in nested cases by using values from outer scope', async () => {
    ...
    expect(muxB.ports.some(p => p.label === "default" && p.connectedSignal.includes('0'))).toBe(true);
    ...
    expect(muxA.ports.some(p => p.label === "default" && p.connectedSignal.includes('0'))).toBe(true);
  });
