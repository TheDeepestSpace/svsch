...
describe('generate region validation', () => {
  ...
  it('marks an arm when an unrelated module port is dragged into its bounds', () => {
    ...
    expect(regionWithExternalPort).toEqual(expect.objectContaining({
      invalid: true,
      warningNote: GENERATE_REGION_EXTERNAL_NODE_WARNING
    }));
    expect([...findExternalBlockIds([regionWithExternalPort], [
      node('owned', 24, 24),
      node('external-port', 96, 96, 'port')
  ...
  it('flags both generate-block wrappers when they overlap', () => {
    ...
    expect(byId.get('wrap-a')).toEqual(expect.objectContaining({ invalid: true, warningNote: GENERATE_REGION_OVERLAP_WARNING }));
    expect(byId.get('wrap-b')).toEqual(expect.objectContaining({ invalid: true, warningNote: GENERATE_REGION_OVERLAP_WARNING }));
  });
  ...
  it('flags a generate-block wrapper and the unrelated block inside it', () => {
    ...
    expect(regions.find((r) => r.id === 'wrap')).toEqual(expect.objectContaining({
      invalid: true,
      warningNote: GENERATE_REGION_EXTERNAL_NODE_WARNING
    }));
