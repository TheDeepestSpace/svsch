...
function port(
  overrides: Partial<PositionedNode['ports'][number]> & {
    id: string;
    name: string;
...
describe('HdlNode render snapshots (no-visual-change guard)', () => {
  ...
  it('register: array node', () => {
    ...
    expect(
      renderNode(node, {
        arrayConnections: [
          { portId: 'd2', role: 'target', thick: true },
          { portId: 'q2', role: 'source' },
        ],
      }),
    ).toMatchSnapshot();
  });
  ...
  it('replicate', () => {
    const node: PositionedNode = {
      ...
      repeatExpression: 'N',
      repeatExpressionSource: { file: 'x.sv', startLine: 1 },
      position: pos,
    } as PositionedNode;
    expect(renderNode(node)).toMatchSnapshot();
  });
  ...
  it('literal', () => {
    const node: PositionedNode = {
      ...
      kind: 'literal',
      label: "8'hFF",
      ports: [port({ id: 'lito', name: 'out', direction: 'output' })],
      position: pos,
    } as PositionedNode;
    expect(renderNode(node)).toMatchSnapshot();
  });
  ...
  it('instance: with parameters and warning', () => {
    const node: PositionedNode = {
      ...
      instanceParameters: [{ name: 'WIDTH', value: '8' }],
      warningNote: 'example warning',
      position: pos,
    } as PositionedNode;
    expect(renderNode(node)).toMatchSnapshot();
  });
  ...
  it('port: input', () => {
    const node: PositionedNode = {
      ...
      kind: 'port',
      label: 'clk',
      ports: [port({ id: 'p1a', name: 'clk', direction: 'input' })],
      position: pos,
    } as PositionedNode;
    expect(renderNode(node)).toMatchSnapshot();
  });
  ...
  it('port: output array', () => {
    const node: PositionedNode = {
      ...
      kind: 'port',
      label: 'y',
      ports: [port({ id: 'p2a', name: 'y', direction: 'output' })],
      isArrayNode: true,
      arrayDimension: '[1:0]',
      position: pos,
    } as PositionedNode;
