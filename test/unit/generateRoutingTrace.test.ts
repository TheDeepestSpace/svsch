...
vi.mock('../../src/layout/libavoidRouter', async (importOriginal) => {
  ...
  return {
    ...original,
    routeDiagramWithLibavoid: async (
      ...args: Parameters<typeof original.routeDiagramWithLibavoid>
    ) => {
...
function allEdges(root: ElkNode): ElkEdge[] {
  return [...(root.edges ?? []), ...(root.children ?? []).flatMap(allEdges)];
}
...
describe('compound generate routing trace', () => {
  ...
  it(
    'keeps Libavoid routes with shared collinear segments without ELK fallback ' + 'routes',
    async () => {
      ...
      const view = await buildViewModel(graph, 'generate_arm_multi_block_top', {
        version: 1,
        modules: {},
      });
      ...
      const cToOther =
        'edge:port:generate_arm_multi_block_top:c:instance:generate_arm_multi_block_top:' +
        'g_if_other.u_other:c';
      const otherToY =
        'edge:instance:generate_arm_multi_block_top:g_if_other.u_other:' +
        'port:generate_arm_multi_block_top:y:y';
      const selToMux =
        'edge:port:generate_arm_multi_block_top:sel:mux:generate_arm_multi_block_top:' +
        'g_if_one_y_src:ternary:sel';
      ...
      expect(
        absoluteNodeGeometry(routing.input, 'port:generate_arm_multi_block_top:c'),
      ).toMatchObject({ x: 0, y: 528 });
      expect(
        absoluteNodeGeometry(routing.output, 'port:generate_arm_multi_block_top:c'),
      ).toMatchObject({ x: 24, y: 388 });
      expect(
        absoluteNodeGeometry(routing.input, 'port:generate_arm_multi_block_top:sel'),
      ).toMatchObject({ x: 0, y: 384 });
      expect(
        absoluteNodeGeometry(routing.output, 'port:generate_arm_multi_block_top:sel'),
      ).toMatchObject({ x: 24, y: 456 });
      ...
      const rawCPoints =
        rawEdges.find((edge) => edge.id === cToOther)?.sections?.flatMap(sectionPoints) ?? [];
      ...
      const avoidResult = await routeDiagramWithLibavoid(
        ...
        (nodeId, portId, includeLeadMargins) =>
          renderedLeadPoint(nodeId, portId, nodesById, nodePositions, includeLeadMargins),
      );
