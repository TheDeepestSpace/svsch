...
describe('gate node extraction (uhdm)', () => {
  it.each([
    ...
    expect(gate.ports.filter((p) => p.direction === 'input').map((p) => p.connectedSignal)).toEqual(
      ['a', 'b'],
    );
    expect(gate.ports.find((p) => p.direction === 'output')?.connectedSignal).toBe('y');
    expect(
      module.edges.some((edge) => edge.source === 'port:top:a' && edge.target === gate.id),
    ).toBe(true);
    expect(
      module.edges.some((edge) => edge.source === 'port:top:b' && edge.target === gate.id),
    ).toBe(true);
    expect(
      module.edges.some((edge) => edge.source === gate.id && edge.target === 'port:top:y'),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    'fuses %s into a single negated-output gate, not a separate inverter',
    async (expr, expectedOp) => {
      const module = await extract(`
      module top(input logic a, input logic b, output logic y);
        assign y = ${expr};
      endmodule
    `);
      ...
      expect(
        gateNodes[0].ports.filter((p) => p.direction === 'input').map((p) => p.connectedSignal),
      ).toEqual(['a', 'b']);
    },
  );
  ...
  it.each([
    ...
    'promotes a source-level XNOR (%s) directly, same as the negation-fused form',
    async (_op, expr) => {
      const module = await extract(`
      module top(input logic a, input logic b, output logic y);
        assign y = ${expr};
  ...
  it(
    'flattens a chain of the same operator into one n-ary gate ' +
      '(a & b & c & d -> one 4-input AND)',
    async () => {
      const module = await extract(`
  ...
  it(
    'never flattens across operator types: (a|b)&c stays OR-feeding-AND, ' + 'two separate gates',
    async () => {
      const module = await extract(`
      module top(input logic a, input logic b, input logic c, output logic y);
        assign y = (a | b) & c;
    ...
    `);
      ...
      expect(
        orGate?.ports.filter((p) => p.direction === 'input').map((p) => p.connectedSignal),
      ).toEqual(['a', 'b']);
      const orOutput = orGate?.ports.find((p) => p.direction === 'output')?.connectedSignal;
      expect(
        andGate?.ports.filter((p) => p.direction === 'input').map((p) => p.connectedSignal),
      ).toEqual([orOutput, 'c']);
      expect(
        module.edges.some((edge) => edge.source === orGate?.id && edge.target === andGate?.id),
      ).toBe(true);
    },
  );
