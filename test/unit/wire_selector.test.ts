...
describe.each(['uhdm'] as const)('wire selector connectivity: %s', (backend) => {
  it('properly connects a wire selector from continuous assignment to a mux', async () => {
    ...
    expect(mod.edges.some(e => e.target === mux.id && e.targetPort === 'sel' && e.signal === 'opcode')).toBe(true);
  });
  ...
  it('properly connects a procedural intermediate selector to a mux', async () => {
    ...
    const muxes = mod.nodes.filter(n => n.kind === 'mux' && n.ports.some(p => p.name === 'sel' && p.connectedSignal === 'instruction[6:0]'));
    expect(muxes).toHaveLength(1);
    const mux = muxes[0];
    ...
    expect(mod.edges.some(e => e.target === mux.id && e.targetPort === 'sel' && e.signal === 'instruction[6:0]')).toBe(true);
  });
});
