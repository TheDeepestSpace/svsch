...
test.describe('nested case visual rendering', () => {
  test('renders sibling nested-case arms with identical labels as distinct literal nodes', async ({
    ...
    const view = await openFixture(
      page,
      'nested_case_literal_collision.sv',
      'auto',
      'nested_case_literal_collision',
    );

    const innerMuxes = view.nodes.filter(
      (node) =>
        node.kind === 'mux' &&
        node.ports.some((port) => port.name === 'sel' && port.connectedSignal === 'sel_inner'),
    );
    ...
    const literalSignals = innerMuxes.map(
      (mux) => mux.ports.find((port) => port.label === "2'b01")?.connectedSignal,
    );
    expect(new Set(literalSignals).size).toBe(2);

    const literalLabels = literalSignals.map(
      (signal) =>
        view.nodes.find(
          (node) =>
            node.kind === 'literal' &&
            node.ports.some(
              (port) => port.direction === 'output' && port.connectedSignal === signal,
            ),
        )?.label,
    );
    ...
    await expect(
      page.locator('[data-node-kind="literal"] .svsch-literal-content', { hasText: "4'hA" }),
    ).toBeVisible();
    await expect(
      page.locator('[data-node-kind="literal"] .svsch-literal-content', { hasText: "4'hB" }),
    ).toBeVisible();
