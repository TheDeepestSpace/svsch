...
test.describe('struct passing visual rendering', () => {
  test('renders thick aggregate edges for structs passed between modules', async ({ page }) => {
    ...
    const structEdgeWidth = await structEdge.evaluate((element) => Number.parseFloat(getComputedStyle(element).strokeWidth));
    const normalEdgeWidth = await page.locator('path.svsch-edge:not(.svsch-edge-interface):not(.svsch-edge-interface-bg):not(.svsch-edge-struct):not(.svsch-edge-struct-bg):not(.svsch-edge-thick)').first().evaluate((element) => Number.parseFloat(getComputedStyle(element).strokeWidth));
