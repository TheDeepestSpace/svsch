...
test.describe('parameterized bus width resolution visual rendering', () => {
  test('resolves a bus breakout into taps sized from a parameterized input width', async ({
    page,
  }) => {
    ...
    await expectGraphAndScreenshot(page, 'param-bus-breakout-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });

  test('resolves a bus composition whose replication count is a parameter expression', async ({
    page,
  }) => {
    await page.setViewportSize({ width: 2200, height: 1000 });
    const view = await openFixture(page, 'param_bus_widths.sv', 'auto', 'param_bus_composition');

    const composition = view.nodes.find(
      (node) =>
        node.kind === 'bus' &&
        node.metadata?.role === undefined &&
        node.ports.some((port) => port.direction === 'output' && port.label === 'imm'),
    );
    const replicate = view.nodes.find((node) => node.kind === 'replicate');

    expect(composition).toBeDefined();
    expect(composition?.ports.find((port) => port.direction === 'output')).toMatchObject({
      width: '[7:0]',
    });
    expect(
      composition?.ports
    ...
    await expectGraphAndScreenshot(page, 'param-bus-composition-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });

  // eslint-disable-next-line max-len
  test('resolves a bus breakout on an interface field sized from an instance-overridden parameter', async ({
    page,
  }) => {
    ...
    await expect(
      modport.locator('.svsch-interface-field-label', { hasText: 'level' }),
    ).toBeVisible();
    ...
    await expect(
      breakoutNode.locator('.svsch-bus-tap-label', { hasText: '[7:0]' }).first(),
    ).toBeVisible();

    await expectGraphAndScreenshot(page, 'param-bus-interface-breakout-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });
});
