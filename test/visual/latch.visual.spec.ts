...
test.describe('latch visual rendering', () => {
  test('renders an S-R latch as a case-selected mux feeding an inferred latch', async ({ page }) => {
    ...
    expect(view.edges.some((edge) => edge.source === mux?.id && edge.target === latch?.id && edge.targetPort === 'd')).toBe(true);
    expect(view.edges.some((edge) => edge.source === latch?.id && edge.target === 'port:sr_latch:q')).toBe(true);
    ...
    await expectGraphAndScreenshot(page, 'latch-sr-canvas.png', { clip: await paddedAllNodesClip(page) });
  });
});
