...
test.describe('interface visual rendering', () => {
  test('renders interface ports and patterned aggregate edges', async ({ page }) => {
    ...
    await expect(
      busPort.locator('.svsch-port-type-label', { hasText: 'simple_if' }).first(),
    ).toBeVisible();
    await expect(
      busPort.locator('.svsch-port-modport-label', { hasText: 'slave' }).first(),
    ).toBeVisible();
    await expect(
      busModport.locator('.svsch-interface-field-right', { hasText: 'valid' }),
    ).toBeVisible();
    await expect(
      busModport.locator('.svsch-interface-field-left', { hasText: 'ready' }),
    ).toBeVisible();

    const tapStyle = await busModport
      .locator('.svsch-interface-field-right', { hasText: 'valid' })
      .first()
      .evaluate((element) => {
    ...
    const patternedView = await openFixture(
      page,
    ...
    await expect(
      linkNode.locator('.svsch-interface-side-left.svsch-interface-side-modport-label'),
    ).toHaveText('master');
    await expect(
      linkNode.locator('.svsch-interface-side-right.svsch-interface-side-modport-label'),
    ).toHaveText('slave');
    ...
    const linkInterfaceRoutes = patternedView.edges
      .filter(
        (edge) =>
          edge.source === 'interface:interface_modport:link' ||
          edge.target === 'interface:interface_modport:link',
      )
    ...
    const interfaceBgStyle = await page
      .locator('path.svsch-edge-interface-bg')
      .first()
      .evaluate((element) => {
        const style = getComputedStyle(element);
        return {
    ...
    await expectGraphAndScreenshot(page, 'interface-patterned-edge-canvas.png', {
      clip: await paddedGraphClip(page),
    });

    const masterLabelMessagePromise = page.waitForEvent('console', (message) =>
      message.text().startsWith('NAVIGATE:'),
    );
    await linkNode
      .locator('.svsch-interface-side-left.svsch-interface-side-modport-label', {
        hasText: 'master',
      })
      .click();
  ...
  test('renders an interface view without modports as a blue breakout', async ({ page }) => {
    ...
    await expect(
      interfaceNode.locator('.svsch-interface-field-label', { hasText: 'data' }),
    ).toBeVisible();
    await expect(
      interfaceNode.locator('.svsch-interface-field-label', { hasText: 'valid' }),
    ).toBeVisible();
    ...
    await expectGraphAndScreenshot(page, 'interface-plain-view-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });
  ...
  test('renders interface modports as dual-sided harnesses', async ({ page }) => {
    ...
    await expect(
      master.locator('.svsch-interface-field-right', { hasText: 'valid' }),
    ).toBeVisible();
    ...
    await expect(master.locator('.svsch-interface-field-label')).toHaveText([
      'clk',
      'data',
      'valid',
      'ready',
    ]);
    await expect(slave.locator('.svsch-interface-field-label')).toHaveText([
      'clk',
      'data',
      'valid',
      'ready',
    ...
    await expectGraphAndScreenshot(page, 'interface-modport-view-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });

  test('renders multi-modport interface instances with separate side taps and top inputs', async ({
    page,
  }) => {
    ...
    const topPortY = await stream
      .locator('.interface-top-port[data-port-id$="clk"]')
      .evaluate((port) => {
        return Number.parseFloat((port as HTMLElement).style.top);
      });
    const topHandleGeometry = await stream
      .locator('.interface-top-port[data-port-id$="clk"] .react-flow__handle-top')
      .first()
      .evaluate((handle) => {
    ...
    const streamNode = view.nodes.find(
      (node) => node.id === 'interface:interface_multi_modport:stream',
    );
    ...
    const leftLabels = stream.locator(
      '.svsch-interface-side-left.svsch-interface-side-modport-label',
    );
    const rightLabels = stream.locator(
      '.svsch-interface-side-right.svsch-interface-side-modport-label',
    );
    await expect(leftLabels).toHaveText(['producer', 'controller']);
    await expect(rightLabels).toHaveText(['consumer', 'monitor']);

    const tapBoxes = await stream
      .locator('.svsch-interface-side-modport-label')
      .evaluateAll((labels) => {
    ...
    await expectGraphAndScreenshot(page, 'interface-multi-modport-instance-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });
  ...
  test('renders alternate multi-modport interface arrangements', async ({ page }) => {
    ...
    await openFixture(
      ...
      'auto',
      'interface_uneven_modport',
    );

    const uneven = page.locator('[data-node-id="interface:interface_uneven_modport:link"]');
    await expect(uneven).toBeVisible();
    await expect(
      uneven.locator('.svsch-interface-side-left.svsch-interface-side-modport-label'),
    ).toHaveText(['producer', 'controller']);
    await expect(
      uneven.locator('.svsch-interface-side-right.svsch-interface-side-modport-label'),
    ).toHaveText(['consumer']);
    ...
    await expectGraphAndScreenshot(page, 'interface-uneven-modport-instance-canvas.png', {
      clip: await paddedGraphClip(page),
    });
    ...
    await openFixture(
      ...
      'interface_modport_arrangements.sv',
      'auto',
      'interface_consumer_fanout',
    );

    const fanout = page.locator('[data-node-id="interface:interface_consumer_fanout:link"]');
    await expect(fanout).toBeVisible();
    await expect(
      fanout.locator('.svsch-interface-side-left.svsch-interface-side-modport-label'),
    ).toHaveText(['producer', 'controller']);
    await expect(
      fanout.locator('.svsch-interface-side-right.svsch-interface-side-modport-label'),
    ).toHaveText(['consumer']);
    await expect(
      page.locator('[data-node-id="instance:interface_consumer_fanout:u_sink0"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-node-id="instance:interface_consumer_fanout:u_sink1"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-node-id="instance:interface_consumer_fanout:u_sink2"]'),
    ).toBeVisible();
    expect(await page.locator('.svsch-edge-junction').count()).toBeGreaterThan(0);
    await expect(page.locator('.svsch-edge-junction-interface').first()).toHaveAttribute(
      'r',
      '6.5',
    );
    await expectGraphAndScreenshot(page, 'interface-consumer-fanout-canvas.png', {
      clip: await paddedGraphClip(page),
    });
    ...
    await openFixture(
      ...
      'interface_modport_arrangements.sv',
      'auto',
      'interface_all_left_modports',
    );

    const allLeft = page.locator(
      '[data-node-id="interface:interface_all_left_modports:request_bus"]',
    );
    const requester = page.locator(
      '[data-node-id="instance:interface_all_left_modports:u_requester"]',
    );
    const arbiter = page.locator('[data-node-id="instance:interface_all_left_modports:u_arbiter"]');
    await expect(allLeft).toBeVisible();
    await expect(
      requester
        .locator('.svsch-port-label', { hasText: 'bus' })
        .locator('.svsch-port-type-suffix-blue'),
    ).toHaveText('{}');
    await expect(
      arbiter
        .locator('.svsch-port-label', { hasText: 'bus' })
    ...
    await expect(
      allLeft.locator('.svsch-interface-side-left.svsch-interface-side-modport-label'),
    ).toHaveText(['requester', 'arbiter']);
    await expect(
      allLeft.locator('.svsch-interface-side-right.svsch-interface-side-modport-label'),
    ).toHaveCount(0);
    await expectGraphAndScreenshot(page, 'interface-all-left-modports-canvas.png', {
      clip: await paddedGraphClip(page),
    });
    ...
    await openFixture(
      ...
      'interface_modport_arrangements.sv',
      'auto',
      'interface_all_right_modports',
    );

    const allRight = page.locator(
      '[data-node-id="interface:interface_all_right_modports:event_bus"]',
    );
    const sink = page.locator('[data-node-id="instance:interface_all_right_modports:u_sink"]');
    const observer = page.locator(
      '[data-node-id="instance:interface_all_right_modports:u_observer"]',
    );
    await expect(allRight).toBeVisible();
    await expect(
      sink.locator('.svsch-port-label', { hasText: 'bus' }).locator('.svsch-port-type-suffix-blue'),
    ).toHaveText('{}');
    await expect(
      observer
        .locator('.svsch-port-label', { hasText: 'bus' })
    ...
    await expect(
      allRight.locator('.svsch-interface-side-left.svsch-interface-side-modport-label'),
    ).toHaveCount(0);
    await expect(
      allRight.locator('.svsch-interface-side-right.svsch-interface-side-modport-label'),
    ).toHaveText(['sink', 'observer']);
    await expectGraphAndScreenshot(page, 'interface-all-right-modports-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });
  ...
  test('renders modules with multiple interface modport ports', async ({ page }) => {
    ...
    const bridgeTopView = await openFixture(
      ...
      'interface_modport_arrangements.sv',
      'auto',
      'interface_dual_modport_bridge',
    );
    await fitGraphView(page);

    const upstream = page.locator(
      '[data-node-id="interface:interface_dual_modport_bridge:upstream"]',
    );
    const downstream = page.locator(
      '[data-node-id="interface:interface_dual_modport_bridge:downstream"]',
    );
    ...
    expect(
      bridgeTopView.nodes.find(
        (node) => node.id === 'interface:interface_dual_modport_bridge:upstream',
      ),
    ).toBeDefined();
    expect(
      bridgeTopView.nodes.find(
        (node) => node.id === 'interface:interface_dual_modport_bridge:downstream',
      ),
    ).toBeDefined();
    expect(bridgeTopView.edges).toEqual(
    ...
    await expectGraphAndScreenshot(page, 'interface-dual-modport-bridge-top-canvas.png', {
    ...
    const downstreamModport = page.locator(
      '[data-node-id="interface_modport:pair_bridge:downstream"]',
    );
    ...
    await expect(
      upstreamPort.locator('.svsch-port-modport-label', { hasText: 'master' }).first(),
    ).toBeVisible();
    await expect(
      downstreamPort.locator('.svsch-port-modport-label', { hasText: 'slave' }).first(),
    ).toBeVisible();
    await expectGraphAndScreenshot(page, 'interface-dual-modport-bridge-module-canvas.png', {
      clip: await canvasClip(page),
    });
  });

  test('renders interface instance scalar outputs with a bottom cap', async ({ page }) => {
    const outputView = await openFixture(
      page,
      'interface_modport_arrangements.sv',
      'auto',
      'interface_output_wire',
    );
    ...
    await expect(
      status.locator('.svsch-interface-bottom-label', { hasText: 'done' }),
    ).toBeVisible();
    await expect(page.locator('[data-node-id="port:interface_output_wire:done"]')).toBeVisible();
    expect(outputView.edges).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
    ...
    await expectGraphAndScreenshot(page, 'interface-output-wire-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });
  ...
  test('renders interface scalar caps without side modports', async ({ page }) => {
    ...
    await expectGraphAndScreenshot(page, 'interface-caps-only-canvas.png', {
      clip: await paddedGraphClip(page),
    });
  });
});
