...
Then('I should see a combinational block', async function (this: BddWorld) {
  await expect(this.webviewPage.locator('[data-node-kind="comb"], [data-node-kind="gate"]')).toBeVisible();
});

Then('I should not see a combinational block', async function (this: BddWorld) {
  await expect(this.webviewPage.locator('[data-node-kind="comb"], [data-node-kind="gate"]')).not.toBeVisible();
});
...
Then('there should be a connection between {string} and the combinational block', async function (this: BddWorld, source: string) {
  ...
  const targetId = await this.webviewPage.locator('html').evaluate(() =>
    document.querySelector('[data-node-kind="comb"], [data-node-kind="gate"]')?.closest('.react-flow__node')?.getAttribute('data-id') ?? null
  );
...
async function findGenerateRegionNodeIdByKind(world: BddWorld, label: string, kind: string | string[]): Promise<string | null> {
  const nodeIds = await generateRegionNodeIds(world, label);
  if (nodeIds.length === 0) return null;
  const kinds = Array.isArray(kind) ? kind : [kind];
  return world.webviewPage.locator('html').evaluate((_element, { ids, nodeKinds }) => {
