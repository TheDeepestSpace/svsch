...
async function persistCliPngSnapshot(world: BddWorld, pngBuffer: Buffer) {
  ...
  const numDiffPixels = pixelmatch(expectedImage.data, actualImage.data, diff.data, width, height, {
    threshold: 0.1,
  });
  if (numDiffPixels > 100) {
    ...
    fs.writeFileSync(
      path.join(resultsDir, `${snapshotName}-expected.png`),
    ...
    throw new Error(
      `CLI PNG snapshot mismatch for "${snapshotName}": ${numDiffPixels} pixels differ.`,
    );
  }
}
